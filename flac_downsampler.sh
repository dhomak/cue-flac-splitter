...
# Function to get just the sample rate
get_sample_rate() {
    local file="$1"
    ffprobe -v quiet -print_format json -show_streams "$file" 2>/dev/null | \
    python3 -c "
...
try:
    ...
    if 'streams' in data:
        for stream in data['streams']:
            if stream.get('codec_type') == 'audio':
                print(stream.get('sample_rate', '0'))
                break
        else:
            print('0')
    else:
...
downsample_file() {
    ...
    # Check if file is already 44.1kHz
    local sample_rate=$(get_sample_rate "$input_file")
    if [ "$sample_rate" = "44100" ]; then
        print_warning "Already at 44.1kHz - skipping"
...
")
    ...
    if [ $skipped -gt 0 ]; then
        print_status "Skipped (already 44.1kHz): $skipped files"
    fi
    if [ $failed -gt 0 ]; then
